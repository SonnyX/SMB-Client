...
namespace SMBLibrary
{
    public partial class NTFileStoreHelper
    {
        public static FileAccess ToCreateFileAccess(AccessMask desiredAccess, CreateDisposition createDisposition)
        {
            ...
            if (((FileAccessMask) desiredAccess & FileAccessMask.FILE_READ_DATA) > 0 || ((FileAccessMask) desiredAccess & FileAccessMask.FILE_READ_EA) > 0 || ((FileAccessMask) desiredAccess & FileAccessMask.FILE_READ_ATTRIBUTES) > 0 || (desiredAccess & AccessMask.MAXIMUM_ALLOWED) > 0 || (desiredAccess & AccessMask.GENERIC_ALL) > 0 || (desiredAccess & AccessMask.GENERIC_READ) > 0)
            {
                result |= FileAccess.Read;
            }

            if (((FileAccessMask) desiredAccess & FileAccessMask.FILE_WRITE_DATA) > 0 || ((FileAccessMask) desiredAccess & FileAccessMask.FILE_APPEND_DATA) > 0 || ((FileAccessMask) desiredAccess & FileAccessMask.FILE_WRITE_EA) > 0 || ((FileAccessMask) desiredAccess & FileAccessMask.FILE_WRITE_ATTRIBUTES) > 0 || (desiredAccess & AccessMask.DELETE) > 0 || (desiredAccess & AccessMask.WRITE_DAC) > 0 || (desiredAccess & AccessMask.WRITE_OWNER) > 0 || (desiredAccess & AccessMask.MAXIMUM_ALLOWED) > 0 || (desiredAccess & AccessMask.GENERIC_ALL) > 0 || (desiredAccess & AccessMask.GENERIC_WRITE) > 0)
            {
